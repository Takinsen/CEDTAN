...
แอปสมมติชื่อ SlowQueue เป็นตัวเดินเรื่องทั้งส่วนนี้ มันคือระบบจองคิวร้านอาหาร ประกอบด้วย
...
<Why>
คนเรียนเรื่อง microservices ใหม่ ๆ มักสรุปว่า monolith คือของโบราณที่ต้องเลิกใช้ คาบนี้พูดตรงข้าม และนี่คือประโยคที่สำคัญที่สุดของทั้งคาบ
</Why>
...
เส้นทางนี้ชัดมาก วันแรก SlowQueue 1.0 มีทีมพัฒนาทีมเดียว พอมาถึง SlowQueue 4.0 มันกลายเป็นห้าทีม — Web UI, Mobile UI, Queue Management, Backend และ Ads
...
ยังมีปัญหาอีกชั้น ทีม Queue อยากใช้ Node.js เพราะขยายง่ายกว่า ทีม Backend อยากใช้ Python เพราะจะทำ ML pipeline

แต่ทั้งระบบเป็น J2EE ก้อนเดียว จะเปลี่ยนภาษาต้องเขียนใหม่หมด — **too big to rewrite**

เมื่อรวมกับข้อที่ว่าธุรกิจเปลี่ยนเร็วมาก และสิ่งที่ต้องการคือขยับจาก <Term en="stability" th="ความนิ่ง ไม่ค่อยเปลี่ยน" /> ไปหา <Term en="agility" th="ความคล่องตัว เปลี่ยนได้ไว" /> คือส่งซอฟต์แวร์ได้ **เร็ว บ่อย และเชื่อถือได้** ก็จะเห็นว่า monolith ไปต่อไม่ไหวแล้ว
...
ทั้งหมดย่อเหลือสี่ข้อ ที่เหลือของคาบนี้คือการขยายสี่ข้อนี้
...
ผลคือแต่ละบริการเลือกภาษาของตัวเองได้ — บริการหนึ่งลอง Python อีกตัวย้ายไป JavaScript อีกตัวเขียน Go
...
ประโยคสำคัญคือ **working independently most of the time** — สังเกตคำว่า *most* ไม่ใช่ *all* ยังมีเวลาที่ต้องคุยกัน เช่นตอนจะเปลี่ยนสัญญาของ API

สิ่งที่ microservices ทำให้เกิดได้
...
<Why>
ทุกคนพูดถึงข้อดีของ microservices ส่วนนี้พูดถึงราคา ซึ่งเป็นส่วนที่ตัดสินว่าควรใช้หรือไม่
</Why>
...
**เกณฑ์ตัดสิน:** เลือก microservices เมื่อสิ่งที่องค์กรได้กลับมา มีค่ามากกว่าราคาของการเป็นระบบกระจาย

ฝั่งราคาขยายได้อีกสี่ข้อ
...
และตอนจบเขียนไว้ว่า *along comes Software-Defined Systems* — สามข้อหลังคือปัญหาที่วิชานี้ทั้งวิชาเกิดมาแก้
...
เกณฑ์คือ **single business capability** ไม่ใช่ชั้นเทคนิค ถ้าแบ่งถูก การเพิ่มฟีเจอร์หนึ่งอย่างควรแตะบริการเดียว
</Quiz>
...
<Why>
สองคำนี้มักถูกพูดคู่กันจนหลายคนคิดว่าเป็นของเดียวกัน ต้องแยกให้ชัด เพราะความเข้าใจผิดนี้ทำให้ทีมคิดว่าตัวเองทำ microservices แล้ว ทั้งที่ยังไม่ได้ทำ
</Why>
...
จากนั้นก็ระบุตรง ๆ ว่า **container ไม่ได้ทำให้เกิด microservices**
...
คาบที่ 2 เทียบ VM กับ container ในเชิงโครงสร้างไปแล้ว คาบนี้เทียบในเชิง **ตัวเลข** โดยให้เหตุผลห้าข้อ
...
<Figure number={5} caption="เวลาที่ใช้สั่ง N ตัวขึ้นพร้อมกัน — วาดใหม่จากกราฟในสไลด์ ค่าเป็นค่าประมาณที่อ่านจากกราฟ ที่มา Zhang et al., IEEE CLOUD 2018">
...
คาบที่ 3 สอนเขียน compose file ไปแล้ว คาบนี้เพิ่มมุมที่สำคัญสำหรับ microservices โดยเฉพาะ
...
คาบนี้ใช้ repository จริงสี่ตัวบน GitHub ของอาจารย์ผู้สอน (ทั้งหมดเป็น GPL v3)
...
สองสิ่งนี้ถูกแยกออกจากกันตลอด และมันไม่เท่ากัน

- **tag ของ source code** เช่น `release-1.3` คือจุดใน git ที่ checkout
- **tag ของ image** เช่น `release-1` คือชื่อที่ติดตอน `docker build`

กรณีที่ต้องระวังเขียนย่อไว้แบบนี้ `(src=release-1.3, img=release-1)` — ถ้า checkout ผิด version แล้ว build จะได้ image ที่ใช้ชื่อถูกแต่ข้างในผิด และจะไปพังตอนรัน เป็นจุดที่สไลด์เตือนซ้ำถึงสองครั้ง
</Callout>
...
และสังเกตว่าเลข `8000` ตรงนี้คือ **พอร์ตของ container** ไม่ใช่พอร์ตที่ map ออก host ตรงตามกฎข้างต้น
...
ตอนนี้ต้องพิมพ์ `docker run` สองครั้งทุกรอบ และต้องจำลำดับด้วย ขั้นต่อไปจึงย้ายไป `natawutn/todo-compose`
...
คำสั่งที่ให้ไว้คือ `docker build -t todo:release-1 .` ซึ่งได้ image ชื่อ `todo:release-1`

แต่ compose file หน้า 41 อ้างถึง `todo:release-1.3`

ถ้าพิมพ์ตามทั้งสองที่ตรง ๆ `docker compose up` จะหา image ไม่เจอแล้วไปดึงจาก Docker Hub แทน ซึ่งไม่มีอยู่จริง
...
คำสั่งเต็มชุดสั่งเครือข่ายพร้อมกับตอนสร้าง container ได้เลย ไม่ต้อง connect ทีหลัง
...
**สอง — `image: "redis"` ไม่ได้ระบุ version** แปลว่าได้ `redis:latest` ซึ่งเปลี่ยนได้ทุกเมื่อ อีกที่หนึ่งเขียนเป็น `image: "redis:7.0.12"` ซึ่งเป็นวิธีที่ถูกต้องกว่า — **ระบุเลข version เสมอ** ไม่งั้นวันหนึ่งระบบจะพังโดยที่ไม่มีใครแก้อะไรเลย
</Callout>

ไฟล์เดียวกันนี้ยังอธิบายอีกมุมหนึ่งได้ คือระบายสีแบ่งเป็นสี่ชั้น — front-end, back-end, database และ network — แล้วชี้ว่าเลข version ของแต่ละบริการอยู่ตรงไหน ประเด็นคือ **ไฟล์เดียวบอกได้ทั้งหมดว่าระบบนี้ประกอบด้วยอะไร รุ่นไหน ต่อกันยังไง**
...
ตัวอย่างคือระบบแบบ Grab หรือ Uber ที่
...
ยังมีรายละเอียดที่ควรสังเกตอีกอย่าง คือ **ไม่ใช่ทุก client ที่เข้าทาง gateway** แอปมือถือยิงผ่าน API Gateway ส่วนเบราว์เซอร์บนคอมพิวเตอร์เปิดหน้าเว็บ (Passenger Web UI, Driver Web UI) ตรง ๆ แล้วให้หน้าเว็บนั้นเป็นฝ่ายเรียกบริการต่อ
...
ตัวอย่างคือหน้ารายละเอียดสินค้าของ Amazon หน้าจอเดียวนั้นต้องดึงข้อมูลจากหกบริการ
...
ลองวางสองภาพคู่กันเพื่อให้เห็นความต่าง ภาพแรกมี Service-1 หนึ่งตัว ภาพที่สอง Service-1 กลายเป็นสามสำเนา

สิ่งที่ **ไม่เปลี่ยน** คือฝั่ง client — มันยังยิงมาที่ gateway ที่อยู่เดิม ไม่รู้เลยว่าข้างในมีกี่ตัว

#### จัดการเมื่อคนล้น

สมมติมีคำขอสี่อันวิ่งเข้ามาหา Service-1 พร้อมกัน แต่กำลังรับได้แค่สอง gateway จึงตอบ **OK** ให้สอง และให้อีกสอง **WAIT**
...
สามสำเนานั้นอยู่คนละที่กันหมด: `10.4.3.1:8756`, `10.4.3.99:4545`, `10.4.3.20:333` — ไม่มีรูปแบบให้เดา
</Why>
...
ตัวอย่างของจริงคือ **consul + consul-template + nginx** และ Kubernetes ในคาบหน้าก็ใช้แนวนี้
...
ทุกอย่างรวบเป็นรายการเดียว ใช้เป็นเช็กลิสต์ได้
...
<Term en="nginx" th="เว็บเซิร์ฟเวอร์โอเพนซอร์สที่ใช้เป็น reverse proxy ได้" /> เป็นตัวเลือกที่ใช้ในคาบนี้ เพราะ
...
ชุดคำสั่งหกบรรทัดข้างบนเขียนเป็น compose file ได้ ซึ่งอ่านง่ายกว่ามากและสั่งขึ้นทีเดียวจบ

<Detail label="compose file ของทั้งระบบ">
...
ไฟล์นี้เขียนขึ้นจากคำสั่งเดิม ไม่ได้อยู่ในสไลด์
</Example>

### ได้อะไรจาก deployment orchestration

บทสรุปมีห้าข้อ ซึ่งอ่านแล้วจะเห็นว่ามันตอบปัญหาที่เจอมาทั้งคาบพอดี
